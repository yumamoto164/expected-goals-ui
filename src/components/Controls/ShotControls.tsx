...
const ShotControls = memo(function ShotControls() {
  ...
  const { selectedTeam, selectedPlayer, bodyPart, assistType, shotType } =
    state;
  ...
  return (
    <div className="flex flex-col gap-3 lg:gap-4 xl:gap-5 p-3 lg:p-4 bg-gray-800 rounded-lg h-full">
      {/* Team toggle */}
      <div>
      ...
      {players.length > 0 && (
        <div>
          ...
          <select
            ...
            className="w-full bg-gray-700 text-white rounded px-2 lg:px-3 py-1.5 lg:py-2 text-xs lg:text-sm border border-gray-600 focus:outline-none focus:border-emerald-500"
          >
